...
def test_safe_to_clean_up(fixture_path):
    csvfile = Path(fixture_path, 'empty.csv').as_posix()
    reader = CsvNumericEventReader(csvfile=csvfile)
    reader.clean_up()
    reader.set_up()
    reader.clean_up()
    reader.clean_up()
...
def test_csv_with_header_line(fixture_path):
    csvfile = Path(fixture_path, 'header_line.csv').as_posix()
    reader = CsvNumericEventReader(csvfile=csvfile)
...
def test_csv_with_no_header_line(fixture_path):
    csvfile = Path(fixture_path, 'no_header_line.csv').as_posix()
    reader = CsvNumericEventReader(csvfile=csvfile)
...
def test_csv_skip_nonnumeric_lines(fixture_path):
    csvfile = Path(fixture_path, 'nonnumeric_lines.csv').as_posix()
    reader = CsvNumericEventReader(csvfile=csvfile)

    # Expect these lines to be skipped as non-numeric.
